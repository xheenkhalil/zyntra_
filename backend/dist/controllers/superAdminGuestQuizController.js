...
const getGuestQuizById = async (req, res) => {
    ...
    try {
        const quizResult = await db_1.default.query('SELECT id, title, category, status, created_at, updated_at, average_rating FROM guest_quizzes WHERE id = $1', [quizId]);
...
const deleteGuestQuiz = async (req, res) => {
    ...
    try {
        ...
        const result = await db_1.default.query('DELETE FROM guest_quizzes WHERE id = $1 RETURNING id', [quizId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ message: 'Guest quiz not found.' });
        }
...
const deleteGuestQuizQuestion = async (req, res) => {
    const { questionId } = req.params;
    try {
        const result = await db_1.default.query('DELETE FROM guest_questions WHERE id = $1 RETURNING id', [questionId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ message: 'Guest quiz question not found.' });
        }
