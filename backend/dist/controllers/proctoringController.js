...
const getExamProctoringBatch = async (req, res) => {
    ...
    `;
        ...
        const detectionChartData = {
            labels: detectionResult.rows.length > 0 ? detectionResult.rows.map((r) => r.type) : ['No Data'],
            data: detectionResult.rows.length > 0 ? detectionResult.rows.map((r) => r.count) : [0],
        };
    ...
    `;
        ...
        const threatChartData = {
            labels: threatResult.rows.length > 0 ? threatResult.rows.map((r) => r.time_label) : ['--'],
            critical: threatResult.rows.length > 0 ? threatResult.rows.map((r) => r.critical) : [0],
            high: threatResult.rows.length > 0 ? threatResult.rows.map((r) => r.high) : [0],
            medium: threatResult.rows.length > 0 ? threatResult.rows.map((r) => r.medium) : [0],
        };
