...
const logAudit = async (action, details, userId, organizationId) => {
    ...
    `;
        // Fire-and-forget
        db_1.default.query(query, [action, details, safeUserId, safeOrgId]);
    }
    catch (err) {
...
const getDashboardStats = async (req, res) => {
    try {
        ...
        const usersThisMonth = await db_1.default.query("SELECT COUNT(*) FROM users WHERE created_at > NOW() - INTERVAL '30 days'");
        const usersLastMonth = await db_1.default.query("SELECT COUNT(*) FROM users WHERE created_at > NOW() - INTERVAL '60 days' AND created_at <= NOW() - INTERVAL '30 days'");
...
const createOrganization = async (req, res) => {
    ...
    try {
        const result = await db_1.default.query('INSERT INTO organizations (name, status) VALUES ($1, $2) RETURNING *', [name, 'pending']);
...
const getAllOrganizations = async (req, res) => {
    try {
        const result = await db_1.default.query('SELECT * FROM organizations ORDER BY created_at DESC');
        res.json(result.rows);
    }
    catch (error) {
...
const updateOrganization = async (req, res) => {
    ...
    const adminUserId = req.user?.userId;
    try {
        const result = await db_1.default.query('UPDATE organizations SET name = $1, updated_at = NOW() WHERE id = $2 RETURNING *', [name, id]);
...
const archiveOrganization = async (req, res) => {
    ...
    const adminUserId = req.user?.userId;
    try {
        const result = await db_1.default.query("UPDATE organizations SET status = 'archived', updated_at = NOW() WHERE id = $1 RETURNING *", [id]);
        if (result.rows.length === 0)
            return res.status(404).json({ message: 'Organization not found' });
...
const unarchiveOrganization = async (req, res) => {
    ...
    const adminUserId = req.user?.userId;
    try {
        const result = await db_1.default.query("UPDATE organizations SET status = 'active', updated_at = NOW() WHERE id = $1 RETURNING *", [id]);
        if (result.rows.length === 0)
            return res.status(404).json({ message: 'Organization not found' });
