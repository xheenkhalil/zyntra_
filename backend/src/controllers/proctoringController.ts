...
const uploadBase64Image = async (base64Image: string, key: string): Promise<string> => {
...
const logAudit = async (action: string, details: string, userId: string | null | undefined, organizationId: string | null | undefined) => {
    try {
...
export const enrollIdentity = async (req: AuthRequest, res: Response) => {
    ...
    try {
        ...
        for (let i = 0; i < base64Images.length; i++) {
            ...
            try {
                await rekognitionClient.send(new IndexFacesCommand({
...
export const getProctoringStatus = async (req: AuthRequest, res: Response) => {
    ...
    try {
        const result = await pool.query(
            'SELECT 1 FROM proctor_profiles WHERE user_id = $1',
            [studentId]
        );
