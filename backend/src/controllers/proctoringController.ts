  ...
  try {
    ...
    try {
      ...
      if (detectionResult.rows.length > 0) {
        detectionChartData = {
          labels: detectionResult.rows.map((r: any) => r.type),
          data: detectionResult.rows.map((r: any) => r.count),
        };
      }
    } catch (chartErr: any) {
    ...
    try {
      ...
      if (threatResult.rows.length > 0) {
        threatChartData = {
          labels: threatResult.rows.map((r: any) => r.time_label),
          critical: threatResult.rows.map((r: any) => r.critical),
          high: threatResult.rows.map((r: any) => r.high),
          medium: threatResult.rows.map((r: any) => r.medium),
        };
      }
    } catch (chartErr: any) {
