...
import {
  ...
  CircularProgress, IconButton, Menu, MenuItem, Dialog, DialogActions,
  DialogContent, DialogTitle, Chip, Snackbar
} from '@mui/material';
...
const SuperAdminDashboard: React.FC = () => {
  ...
  const [centralAdminForm, setCentralAdminForm] = useState({ fullName: '', email: '', username: '' });
  const [setupLink, setSetupLink] = useState('');

  // NEW: for email sending
  ...
  const handleCreateOrg = async () => {
    setDialogError('');
    if (!newOrgName.trim()) return setDialogError('Organization name is required.');
    try {
  ...
  const handleCreateCentralAdmin = async () => {
    ...
    if (!centralAdminForm.fullName.trim() || !centralAdminForm.email.trim() || !centralAdminForm.username.trim()) {
      return setDialogError('All admin fields are required.');
    }
    const adminData = { ...centralAdminForm, organizationId: newlyCreatedOrg.id };
    try {
  ...
  if (error)
    return (
      <Container maxWidth="sm" sx={{ mt: 8 }}>
        ...
        <Button variant="contained" onClick={fetchOrgs}>Retry</Button>
      </Container>
    );
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box>
          ...
          <Typography>Welcome, {user?.fullName || 'Super Admin'}!</Typography>
        </Box>
        <Box>
          <Button
            variant="contained"
            ...
            {organizations.length > 0 ? (
              organizations.map((org) => (
                <TableRow key={org.id} hover>
                  ...
                  <TableCell>{org.tier || 'N/A'}</TableCell>
              ...
              <TableRow>
                <TableCell colSpan={5} align="center">No organizations found.</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
      ...
      <Dialog open={createOpen} onClose={handleCloseCreateModal} fullWidth maxWidth="sm">
        <DialogTitle>
          {modalStep === 1 && 'Step 1: Create New Organization'}
          {modalStep === 2 && `Step 2: Create Central Admin for "${newlyCreatedOrg?.name}"`}
          ...
          {modalStep === 2 && (
            <>
              ...
              <TextField
                ...
                onChange={(e) => setCentralAdminForm({ ...centralAdminForm, fullName: e.target.value })}
              />
              <TextField
                margin="dense"
                label="Admin's Email"
                ...
                onChange={(e) => setCentralAdminForm({ ...centralAdminForm, email: e.target.value })}
              />
              <TextField
                margin="dense"
                ...
                onChange={(e) => setCentralAdminForm({ ...centralAdminForm, username: e.target.value })}
              />
            </>
          )}
          {modalStep === 3 && (
            <Box sx={{ mt: 2 }}>
              <Typography variant="body2" sx={{ mb: 1 }}>
                Send this secure link to the new Central Admin. It expires in 24 hours.
              </Typography>
              <TextField
                fullWidth
                variant="outlined"
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          {modalStep < 3 && <Button onClick={handleCloseCreateModal}>Cancel</Button>}
          {modalStep === 1 && <Button onClick={handleCreateOrg} variant="contained">Next</Button>}
          {modalStep === 2 && <Button onClick={handleCreateCentralAdmin} variant="contained">Create Admin & Generate Link</Button>}
          {modalStep === 3 && (
            <>
              ...
              <Button onClick={handleCloseCreateModal} variant="contained">Done</Button>
            </>
          )}
        </DialogActions>
      </Dialog>
